...
5. Before implementing schema models, record the chosen validation system and optional dependency groups in `pyproject.toml` and `README.md`; do not let schema code, docs, and packaging diverge.
6. Document that real network crawling and paid LLM/ScrapeGraphAI calls are optional execution modes; tests must not require credentials.
...
1. Run deterministic offline tests by default: `pytest`. Optional live/network/browser/provider tests must be skipped unless explicit opt-in flags or environment variables are supplied.
...
Regardless of `$ultragoal` or `$team`, implement the first slice in this order before live crawling: schema/evidence/warning models -> fixture fetcher -> one offline fixture pipeline -> Markdown/JSON report smoke. This proves the evidence contract and report shape before optional crawl4ai/browser/PDF/LLM dependencies are introduced.
...
- Lane C (`executor`): classifier only after Phase 1 schema/evidence contracts are stable.
- Lane D (`executor`): extractors/normalizer after schema/evidence and classifier contracts are available.
- Lane E (`test-engineer`): fixtures and acceptance tests.
- Lane F (`writer` or `executor`): CLI/report/README.
...
- Applied Critic mandatory improvements: test-spec acceptance matrix now includes adapter-contract, incremental diff, failure recovery, and optional dependency behavior; PRD verification clarifies offline-only default tests; Phase 0 requires documenting validation/dependency decisions before schema implementation.
- Applied Critic optional improvements: first implementation slice recommendation added; Team lanes split classifier and extractor/normalizer dependencies to reduce parallel coordination risk.
