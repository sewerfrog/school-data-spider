...
- Missing reasons：对 `coverage.missing` 增加字段级 `missing_reasons`，用于说明当前 crawl/extractor 链路卡在 `not_attempted`、`attempted_no_match`、`context_gate_failed`、`application_portal_unreachable` 等位置。同一字段同时存在 extractor `no_match` 和 context gate skipped 时，当前优先展示 `attempted_no_match`；它仍不是官网缺失证明。
...
结果：pytest 为 `114 passed`；compileall 通过。
